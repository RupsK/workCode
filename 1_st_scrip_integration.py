# -*- coding: utf-8 -*-
"""
Created on Mon Jul 29 10:55:29 2024
...
# Directory paths
image_dir = "C:/Users/h4tec/Desktop/jejuPC41"
json_file = "H:/PC POLE IMAGES/output_2_json.json" 
...
def process_image(image_file):
    ...
    # Use YOLO model to detect holes
    hole_results = model.predict(cropped_image_file, conf=0.01, iou=0.3)

    # Extract hole detection results
    holes = []
    ...
    # Save hole detection results to a JSON file
    predicted_json_file = os.path.join(output_dir, f'{os.path.splitext(image_file)[0]}_holes.json')
    with open(predicted_json_file, 'w', encoding='utf-8') as f:
        json.dump({"holes": holes}, f, ensure_ascii=False, indent=4)
    ...
    return touch_count, adjusted_center_list, holes, cropped_image_file
...
for image_file in image_files:
    touch_count, adjusted_center_list, holes, cropped_image_file = process_image(image_file)
...
# Print the selected views
logging.info(f"Front view image: {front_view_image}")
logging.info(f"Left view image: {left_view_image}")
logging.info(f"Right view image: {right_view_image}")

# Save the selected views to the "views" folder
front_view_path = os.path.join(views_dir, os.path.basename(front_view_image))
...
os.rename(os.path.join(image_dir, front_view_image), front_view_path)
...
# Create subdirectories for each view and save the corresponding cropped images
for view_image in [front_view_image, left_view_image, right_view_image]:
    view_image_id = os.path.splitext(os.path.basename(view_image))[0]
    view_cropped_dir = os.path.join(views_dir, "cropped", view_image_id)
    os.makedirs(view_cropped_dir, exist_ok=True)
    
    cropped_image_path = os.path.join(cropped_images_dir, f'{view_image_id}_cropped.jpg')
    if os.path.exists(cropped_image_path):
        os.rename(cropped_image_path, os.path.join(view_cropped_dir, f'{view_image_id}_cropped.jpg'))

logging.info(f"Cropped images for selected views saved to {os.path.join(views_dir, 'cropped')}")
